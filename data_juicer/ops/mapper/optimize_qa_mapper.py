...
class OptimizeQAMapper(Mapper):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        :param input_template: Template for building the input for the model.
            Supports positional placeholder '{}' or named '{qa_pair}'.
        :param qa_pair_template: Template for formatting the question and
            answer pair. Supports positional placeholders '{}' (question
            first, answer second) or named '{question}' and '{answer}'.
    ...
    def build_input(self, sample):
        question = sample[self.query_key]
        answer = sample[self.response_key]

        if "{question}" in self.qa_pair_template or "{answer}" in self.qa_pair_template:
            qa_pair = self.qa_pair_template.format(question=question, answer=answer)
        else:
            qa_pair = self.qa_pair_template.format(question, answer)

        if "{qa_pair}" in self.input_template:
