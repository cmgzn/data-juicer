...
class LabelStudioAnnotationMapperTest(DataJuicerTestCaseBase):
    ...
    def setUp(self):
        ...
        self.mock_ls_project.get_task.return_value = self.mock_completed_tasks[0]
    ...
    def test_initialization_success(self, MockLabelStudioClient):
        """Test successful initialization of LabelStudioAnnotationMapper"""
        ...
        self.assertEqual(mapper.project_id, 999)
        self.assertEqual(mapper.samples_per_task, 1)
        MockLabelStudioClient.assert_called_once_with(url="http://example.com", api_key="test_key")
        self.mock_ls_client.get_project.assert_called_once_with(999)
    ...
    @patch("label_studio_sdk.Client")
    def test_url_generation(self, MockLabelStudioClient):
        """Test URL generation methods"""
        MockLabelStudioClient.return_value = self.mock_ls_client
        mapper = MockLabelStudioAnnotationMapper(
            api_url="http://example.com/api", api_key="test_key", project_name="Test Project", project_id=999
        )
        self.assertEqual(mapper._get_project_url(), "http://example.com/projects/999")
        self.assertEqual(mapper._get_task_url(123), "http://example.com/projects/999/data?task=123")
    ...
    @patch("label_studio_sdk.Client")
    def test_get_all_annotations(self, MockLabelStudioClient):
        MockLabelStudioClient.return_value = self.mock_ls_client
        mapper = MockLabelStudioAnnotationMapper(
            api_url="http://example.com", api_key="test_key", project_name="Test Project", project_id=999
        )
